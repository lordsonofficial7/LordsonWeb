...
export default function Hero() {
  ...
  useEffect(() => {
    const fetchBannerData = async () => {
      const data = await getBanners();
      if (data && data.length > 0) {
        // Filter banners with odd id
        const oddBanners = data.filter((b) => b.id % 2 !== 0);
        // Pick active odd banner or first odd banner
        const activeOddBanner = oddBanners.find((b) => b.is_active) || oddBanners[0];
        setBanner(activeOddBanner);
      }
      setLoading(false);
    };
  ...
  return (
    <section
      className="relative left-0 w-full h-[calc(100vh-80px)] text-white overflow-hidden"
    >
      {/* Background image with blur + transparency */}
      <div
        className="absolute inset-0 z-0"
